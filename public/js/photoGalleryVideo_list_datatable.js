...
var KTDatatablesBasicPaginations = function() {
	var initTable1 = function() {
		var table = $('#kt_datatable_photovideo_list');
		var $i=1;
		var jsonURL = $('#urlListData').attr('data-info');
		var crudUrlTemplate = JSON.parse(jsonURL);
		// var columnList = JSON.parse(table.data('columnListData'));
		//var crudUrlTemplate = JSON.parse(table.data('crudUrlTemplate'));
		...
		var tableObject = table.DataTable({
			...
			columnDefs: [
...

				{
					...
					render: function(data, type, full ,meta){
						var type = {
							"0": { 'title': 'Photo', 'class': 'badge badge-light-warning fw-bold' },
							"1": { 'title': 'Video', 'class': 'badge badge-light-success fw-bold' }
						};
						if (typeof type[data] === 'undefined') {
							return data;
						}
						return '<span class="' + type[data].class + '">' + type[data].title + '</span>';
                    }
				},
				{
					targets: -2,
